...
(function (Skyward) {
    ...
    function build() {
        ...
        Skyward.Hare.generateSprites(txtHare);
        Skyward.game = new Skyward.f.Node("Game");
        Skyward.hare = new Skyward.Hare("Hare");
        ...
        Skyward.game.appendChild(Skyward.hare);
        ...
        function update(_event) {
            ...
            Skyward.camera.cmpTransform.local.translation = new Skyward.f.Vector3(Skyward.hare.cmpTransform.local.translation.x, Skyward.hare.cmpTransform.local.translation.y, Skyward.hare.cmpTransform.local.translation.z);
            Skyward.viewport.draw();
            //muteSounds();
    ...
    */
    ...
    function normalizeTransforms(rotDirection) {
        Skyward.hare.cmpTransform.local.rotateY(rotDirection);
        ...
        for (let j = 0; j <= NodeArray.length - 1; j++) {
            ...
            //for (let m = 0; m <= Vector3Array.length - 1; m++)
            {
                ...
                if (rotation == 90 || rotation == -90) {
                    ...
                    Skyward.hare.cmpTransform.local.translateX(-Skyward.hare.cmpTransform.local.translation.x);
                    // hare.cmpTransform.local.translation.x = 0;
                    NodeArray[j].cmpTransform.local.translateZ(Skyward.Vector3Array[i].z);
                    if (i == 0 && j == 0)
                        Skyward.hare.cmpTransform.local.translateZ(Skyward.Vector3Array[Skyward.hare.lastHitIndex].z);
                }
                if (rotation > -40 && rotation < 40 || rotation == 180 || rotation == -180) { // hare.cmpTransform.local.translation.x = hare.lastHit.x;
                    NodeArray[j].cmpTransform.local.translateZ(-Skyward.Vector3Array[i].z);
                    Skyward.hare.cmpTransform.local.translateZ(-Skyward.hare.cmpTransform.local.translation.z);
                    if (rotation == 180) {
                    ...
                    NodeArray[j].cmpTransform.local.translateX(Skyward.Vector3Array[i].x);
                    if (i == 0 && j == 0) {
                        Skyward.hare.cmpTransform.local.translateX(Skyward.Vector3Array[Skyward.hare.lastHitIndex].x);
                    }
                }
